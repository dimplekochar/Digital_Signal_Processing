 ...
 main( void )
{
    ...
    int wavelength[25]={92, 92, 82, 92, 69, 73, 92, 92, 82, 92, 61, 69, 92, 92, 46, 55, 69, 73, 82, 52, 52, 55, 69, 61, 69};
    int no_of_cycles[25]={52, 52, 118, 105, 140, 132, 52, 52, 118, 105, 157, 280, 52, 52 , 210, 176, 140, 132, 118, 93, 93, 176, 140, 157, 280};
	...
	while(1){
   for (x=0; x<25; x++) {
    	for(y=0;y<no_of_cycles[x];y++){
    		for(z=0;z<wavelength[x]/2;z++){
    			while((Xmit & I2S0_IR)==0); // Wait for transmit interrupt to be pending
I2S0_W0_MSW_W=(32767/wavelength[x])*2*z;
I2S0_W1_MSW_W=(32767/wavelength[x])*2*z;
    	}
    		for(z=(wavelength[x]/2)-1;z>=0;z--){
    			while((Xmit & I2S0_IR)==0); // Wait for transmit interrupt to be pending
I2S0_W0_MSW_W=(32767/wavelength[x])*2*z;
I2S0_W1_MSW_W=(32767/wavelength[x])*2*z;
    	}
