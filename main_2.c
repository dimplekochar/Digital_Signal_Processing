 ...
 * ------------------------------------------------------------------------ */
 main( void )
{
    Int16 LEFT_DATA,RIGHT_DATA, a, b, c, d;
    /* Initialize BSL */
    SYS_EXBUSSEL = 0x6100;
    USBSTK5515_init( );
    ...
    while(TRUE)
    {
	    ...
	    while((Rcv & I2S0_IR) == 0);  // Wait for interrupt pending flag
	    c = I2S0_W0_MSW_R;  // 16 bit left channel received audio data
	    d = I2S0_W1_MSW_R;  // 16 bit right channel received audio data
	        
	    while((Xmit & I2S0_IR) == 0);
	    a = LEFT_DATA;  // 16 bit left channel transmit audio data
	    b = RIGHT_DATA; // 16 bit right channel transmit audio data
